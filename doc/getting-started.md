...
The Dogmcoin Core installation provides the `dogmcoin-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `22982`, so that other tools and libraries can interact with it.
...
You may need to specify `-datadir` also when using `dogmcoin-cli`.
