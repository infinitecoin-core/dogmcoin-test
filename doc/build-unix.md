...
You must install required dependencies to build a basic Dogmcoin daemon, optional dependencies may vary according to your requirements.
...
Dogmcoin Core's released binaries are built using the "depends system", which
...
**Optional dependencies** :  
```bash
# Qt (required for dogmcoin-qt GUI)
...
According to installed dependencies, the following steps will compile `dogmcoind`, `dogmcoin-cli` and `dogmcoin-qt`.
...
#### Enable Qt GUI
Create `dogmcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Dogmcoin Core expect BerkeleyDB 5.3**.
...
#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Dogmcoin may be compiled in
...
#### Security
To help make your Dogmcoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Dogmcoin should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Dogmcoin Core.
