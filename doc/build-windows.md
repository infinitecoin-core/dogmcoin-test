...
Below are some notes on how to build Dogmcoin Core for Windows.
...
Note that for WSL v1 the Dogmcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/dogmcoin, AND not under, for example, /mnt/d/dogmcoin. 
...
way. This will install to `c:\workspace\dogmcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/dogmcoin
